...
DECLSPEC_IMPORT int __cdecl    MSVCRT$system(char const* Command);
...
DECLSPEC_IMPORT BOOL    WINAPI KERNEL32$DeviceIoControl(HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuffer, DWORD nInBufferSize, LPVOID lpOutBuffer, DWORD nOutBufferSize, LPDWORD lpBytesReturned, LPOVERLAPPED lpOverlapped);
...
DECLSPEC_IMPORT NTSTATUS NTAPI NTOSKRNL$NtFsControlFile(HANDLE FileHandle, HANDLE Event, PIO_APC_ROUTINE  ApcRoutine, PVOID ApcContext, PIO_STATUS_BLOCK IoStatusBlock, ULONG FsControlCode, PVOID InputBuffer, ULONG InputBufferLength, PVOID OutputBuffer, ULONG OutputBufferLength);
...
/*other*/
#define NtQueryVirtualMemory RNTOSKRNL$NtQueryVirtualMemory
#define RtlInitUnicodeString RNTOSKRNL$RtlInitUnicodeString
...
#define NtFsControlFile		NTOSKRNL$NtFsControlFile
